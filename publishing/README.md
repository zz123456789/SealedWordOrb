...
当前两个版本已被上传接口接收：MIT 更新后的 0.1.0 文件 ID 为 8940630，1.0.0 文件 ID 为 8940633。审核状态及主页正文尚未通过网页验证；浏览器连接仍不可用。

没有创建后台定时任务；后续更新可按请求复用这些脚本同步上传。

## 许可

原创源码、资源和文档采用 MIT；授权追溯适用于之前发布的版本，详见根目录 LICENSING.md。两个发布包内含 LICENSE 和 LICENSING.md，mods.toml 声明 MIT。CurseForge 旧文件说明已更新为 MIT 授权和新版下载链接；网站项目许可选择器仍需核实。
